...
from nerf.adv.camera import transform_rays
...
def anneal_nearfar(it, near_final, far_final,
                   n_steps=512, init_perc=0.2, mid_perc=0.5):
...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        if aabb_box is None:
            ...
            aabb_train = torch.FloatTensor(aabb_box)   #y -0.5 0.5 ship, -0.6 hotdog
    ...
    def color(self, x, d, mask=None, current_iter=None, total_iter=None, start_ptr=1, **kwargs):
        raise NotImplementedError()
    ...
    def run(self, rays_o, rays_d, cam_id=None, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, adv_perturb={}, num_rays=4096, current_iter=None, total_iter=None, start_ptr=1, **kwargs):
        ...
        if adv_cam_r is not None and adv_cam_t is not None:
            rays_o, rays_d = transform_rays(rays_o.clone(), rays_d.clone(), cam_id, adv_cam_r, adv_cam_t, num_rays) #select only training rays to perturb
        ...
        with torch.no_grad():
            ...
            aabb = aabb.to(device=device)
            # sample steps
            nears, fars = raymarching.near_far_from_aabb(rays_o, rays_d, aabb, self.min_near)
        nears.unsqueeze_(-1)
        fars.unsqueeze_(-1)

        if self.training and kwargs.get('anneal_nearfar'):
            nears, fars = anneal_nearfar(current_iter, nears, fars, kwargs.get('anneal_nearfar_steps'), kwargs.get('anneal_nearfar_perc'), kwargs.get('anneal_mid_perc'))
        ...
        # Adv perturb on point depth
        perturb_pts, perturb_zs = adv_perturb.get('pts_c', None), adv_perturb.get('zval_c', None)
        if perturb_zs is not None:
            z_vals[:num_rays] = z_vals[:num_rays] + perturb_zs
        ...
        if perturb_pts is not None:
            xyzs[:num_rays] = xyzs[:num_rays] + perturb_pts
        ...
        if perturb_raw is not None:
            density_outputs['sigma'][:num_rays] = density_outputs['sigma'][:num_rays] + perturb_raw
        ...
        if upsample_steps > 0:
            ...
            if perturb_zs is not None:
                new_z_vals[:num_rays] = new_z_vals[:num_rays] + perturb_zs
            ...
            # Adv noise on point coordinates
            if perturb_pts is not None:
                new_xyzs[:num_rays] = new_xyzs[:num_rays] + perturb_pts

            # only forward new points to save computation
            new_density_outputs = self.density(new_xyzs.reshape(-1, 3), perturb_feat=adv_perturb.get('feat_f', None), current_iter=current_iter, total_iter=total_iter, start_ptr=start_ptr)
            #new_sigmas = new_density_outputs['sigma'].view(N, upsample_steps) # [N, t]
            for k, v in new_density_outputs.items():
                new_density_outputs[k] = v.view(N, upsample_steps, -1)

            # Adv perturb on re-rendering
            perturb_raw = adv_perturb.get('raw_f', None)
            if perturb_raw is not None:
                new_density_outputs['sigma'][:num_rays] = new_density_outputs['sigma'][:num_rays] + perturb_raw
        ...
        if adv_rgb is not None:
            image[:num_rays] = image[:num_rays] + adv_rgb
    ...
    def render(self, rays_o, rays_d, cam_id = None, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], cam_id, **kwargs)
            ...
            results = _run(rays_o, rays_d, cam_id=cam_id, **kwargs)
