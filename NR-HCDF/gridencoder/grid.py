...
# 尝试导入内部实现模块 _gridencoder，如果失败则从相对路径导入 backend 模块中的 _backend
try:
...
class _grid_encode(Function):
    ...
    def forward(ctx, inputs, embeddings, offsets, per_level_scale, base_resolution, calc_grad_inputs=False, gridtype=0, align_corners=False, interpolation=0):
        ...
        inputs = inputs.contiguous()  # 将输入的张量转换为连续性张量

        B, D = inputs.shape # batch size, coord dim
        L = offsets.shape[0] - 1 # level
        C = embeddings.shape[1] # embedding dim for each level 嵌入参数的维度信息
        S = np.log2(per_level_scale) # resolution multiplier at each level, apply log2 for later CUDA exp2f
        H = base_resolution # base resolution 基础分辨率

        # manually handle autocast (only use half precision embeddings, inputs must be float for enough precision)
        # if C % 2 != 0, force float, since half for atomicAdd is very slow.
        if torch.is_autocast_enabled() and C % 2 == 0:
            embeddings = embeddings.to(torch.half)  # 将embeddings张量的数据类型转换为torch.half

        # L first, optimize cache for cuda kernel, but needs an extra permute later
        outputs = torch.empty(L, B, C, device=inputs.device, dtype=embeddings.dtype)  # 这个张量的形状是(L, B, C) outputs被初始化为空张量
        ...
        # .reshape(B, L * C)将变换后的张量进一步重塑为形状为(B, L * C)的张量 permute(1, 0, 2)将outputs的维度顺序从(B, L, C)变为(L, B, C)
...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear', device="cpu"):
        super().__init__()
        ...
        self.input_dim = input_dim  # coord dims, 2 or 3 输入维度
        self.num_levels = num_levels  # num levels, each level multiply resolution by 2金字塔级别的数量，默认为16，表示将创建这么多不同分辨率的层级
        self.level_dim = level_dim  # encode channels per level 每个级别上的编码通道数
        self.per_level_scale = per_level_scale  # multiply resolution by this scale at each 每一级的缩放比例，默认为2，意味着每上升一级，分辨率将乘以2
        self.log2_hashmap_size = log2_hashmap_size
        self.base_resolution = base_resolution
        self.output_dim = num_levels * level_dim
        self.gridtype = gridtype  # 网格类型
        ...
        offsets.append(offset)
        offsets = torch.from_numpy(np.array(offsets, dtype=np.int32)).to(device=device)
        self.register_buffer('offsets', offsets)
        
        self.n_params = offsets[-1] * level_dim

        # parameters
        self.embeddings = nn.Parameter(torch.empty(offset, level_dim, device=device))
