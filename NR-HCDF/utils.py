...
class Trainer(object):
    def __init__(self, 
        ...
        # print("tiaoshi111111111111111111111111111111111111111111111111111111111111111111111")
        self.name = name
        self.opt = opt
        self.mute = mute
        ...
        if lr_scheduler is None:
            ...
            self.lr_scheduler_1 = lr_scheduler(self.optimizer_1)
    ...
    def train_step(self, data, adv_perturb={}):
        ...
        # for freq_reg_mask after input encoding and anneal near-far
        ...
        outputs_clean = self.model.render(rays_o, rays_d, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if self.opt.patch_size == 1 else True, current_iter=current_iter, total_iter=total_iter, **vars(self.opt)) #without adv
        ...
        if self.opt.patch_size > 1:
            ...
            pred_rgb_clean = pred_rgb_clean.view(-1, self.opt.patch_size, self.opt.patch_size, 3).permute(0, 3, 1, 2).contiguous()

            # torch_vis_2d(gt_rgb[0])
            # torch_vis_2d(pred_rgb[0])

            # LPIPS loss [not useful...]
            loss = loss + 1e-3 * self.criterion_lpips(pred_rgb_clean, gt_rgb)
            ???
            '''
        ...
        loss = loss + 0.0001 * 1 / (self.model.threshold_2 + 1e-6)
    ...
    def distortion_loss(self, z_vals, weights, depth):
        # print("tiaoshi44444444444444444444444444444444444444444444444444444444444444444444444")
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            self.scaler.step(self.optimizer_1)
            self.scaler.update()
    ...
    def train_one_epoch(self, loader):
        self.log(f"==> Start Training Epoch {self.epoch}, lr={self.optimizer.param_groups[0]['lr']:.6f} ...")  # 显示训练的当前轮次（epoch）和学习率
        total_loss = 0
