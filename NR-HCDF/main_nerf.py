...
if __name__ == '__main__':
    ...
    if opt.test:
        ...
        thrd = []
        thrd.append(threshold_2)
        thrd.append(tau)
        thrd.append(ratio)
        thrd.append(ratio2)
        optimizer_1 = torch.optim.Adam(params=thrd, lr=5e-4, betas=(0.9, 0.999))
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace,
                          optimizer=optimizer, optimizer_1=optimizer_1, criterion=criterion, ema_decay=0.95, fp16=opt.fp16,
                          lr_scheduler=scheduler, scheduler_update_every_step=True, metrics=metrics,
                          use_checkpoint=opt.ckpt, eval_interval=50, awp_adversary=awp_adversary)
